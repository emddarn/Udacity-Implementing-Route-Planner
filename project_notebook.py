...
show_map(map_40, start=5, goal=34, path=[5,32,29,22,34])
...
def create_closedSet(self):
    ...
    self.closedSet = set() #Creating closedSet
...
def create_openSet(self):
    ...
    if self.start != None:
        ...
        self.openSet = set() #creating openSet
        self.openSet.add(start) #Adding the start node to openSet
        return self.openSet
...
def create_cameFrom(self):
    ...
    self.cameFrom = {}  #Creating an empty dictionary to store values of explored places
...
def create_gScore(self):
    ...
    self.gScore = list(set())  #Creating an empty list to store gScore
    self.gScore.append(0)  #Initializing the start node with a gScore of 0
    return self.gScore
...
def create_fScore(self):
    ...
    self.fScore = list(set())  #Creating an empty list to store fScore
    self.fScore.append(heuristic_cost_estimate(start, goal)) #Initializing the fScore at Start as completely heuristic
    return self.fScore
...
def set_map(self, M):
    ...
    self.map = {}      #Creating a new map as an empty dictionary to store different points.
    return self.map
...
def set_start(self, start):
    ...
    self.start = start    #Setting the Start value
    self.goal = None      #Removing goal, closedSet, openSet, cameFrom, gScore, fScore, and path attributes' values.
    self.closedSet = None
    self.openSet = None
    self.cameFrom = None
    self.gScore = None
...
def set_goal(self, goal):
    ...
    self.goal = goal     #Setting goal value
    return self.goal
...
def get_current_node(self):
    ...
    self.fScore.append(min(self.openSet))  #Appending the fScore list by getting the minimum value from the openSet
    return self.fScore
...
def get_neighbors(self, node):
    ...
    self.get_neighbors = M.roads[node]
    return self.get_neighbors #Return the neighbor for the current node calculated in the get_current_node method
...
def get_gScore(self, node):
    ...
    self.get_gScore = create_gScore(self,node)  #Work on this
    return self.get_gScore
...
def get_tenative_gScore(self, current, neighbor):
    ...
    self.get_tenative_gScore = self.get_gScore[current] + self.path(current,neighbor)
    return self.get_tenative_gScore
...
def is_open_empty(self):
    ...
    if len(self.openSet) == 0: #Checking to see if openSet is empty
        return True
    else:
        return False
...
def distance(self, node_1, node_2):
    ...
    self.distance = sqrt(((self.node_1.intersections[0][0] - self.node_2.intersections[0][0])**2) + ((self.node_1.intersections[0][1] - self.node_2.intersections[0][1])**2))
...
def heuristic_cost_estimate(self, node):
    ...
    self.heuristic_cost_estimate = self.distance(self.node, self.goal)
    return self.heuristic_cost_estimate
...
def calculate_fscore(self, node):
    ...
    self.calculate_fScore = self.get_gScore + self.heuristic_cost_estimate
    self.fScore.append(self.calculate_fScore)
    return self.calculate_fScore
